...
pub struct McpServerStdio {
    ...
    #[serde(default)]
    pub trusted: bool,
    /// Whether this server was generated by the Buzz desktop's MCP registry,
    /// and so runs the bundled `buzz-mcp-launch` rather than a command an
    /// operator wrote.
    ///
    /// It buys exactly one thing: `mcp::spawn_one` forwards this process's
    /// `BUZZ_MCP_CAPABILITY` -- the per-agent, per-generation token the
    /// launcher must present to read an `mcp:` reference out of the keychain
    /// -- into the child's environment. Every other MCP child is spawned
    /// without it, so an operator-declared server cannot borrow the agent's
    /// authority over its registry credentials.
    ///
    /// `#[serde(default)]` makes the marker absent mean "not from the
    /// registry", which is the fail-closed reading: a client that does not
    /// send it gets nothing forwarded, and a registry server it did not
    /// declare simply fails to resolve.
    #[serde(default)]
