  ...
  return (
    <>
      ...
      <Section title="The words">
        <div className="flex flex-col gap-0 rounded-lg border border-secondary bg-panel px-5 py-2">
          {VOCABULARY.map((group) => (
            ...
            >
              <span className="w-24 shrink-0 text-xs font-semibold text-tertiary">
                {group.group}
              </span>
              <div className="flex min-w-0 flex-1 flex-wrap gap-x-2 gap-y-1">
                {group.words.map((word) => (
                  <code
                    key={word}
                    className="rounded bg-inset px-1.5 py-0.5 text-xs text-primary"
                  >
                    {word}
                  </code>
                ))}
      ...
      >
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="flex-1 rounded-lg border border-success bg-success-tint px-4 py-3">
            <p className="mb-1.5 text-xs font-semibold text-success">Legal</p>
            {GRAMMAR_EXAMPLES.legal.map((example) => (
              <code key={example} className="block text-xs text-success">
                {example}
              </code>
            ))}
          </div>
          <div className="flex-1 rounded-lg border border-danger bg-danger-tint px-4 py-3">
            <p className="mb-1.5 text-xs font-semibold text-danger">Illegal</p>
            {GRAMMAR_EXAMPLES.illegal.map((example) => (
              <code key={example} className="block text-xs text-danger">
                {example}
              </code>
            ))}
          </div>
