...
desktop-standalone *ARGS: _ensure-sidecar-stubs
    ...
    # buzz-mcp-launch is on the list because generated MCP config names it by
    # absolute path: with only the 0-byte stub that _ensure-sidecar-stubs
    # creates, every registry server on a standalone run fails to execute.
    for bin in buzz-acp buzz-agent buzz-backend-kubernetes buzz-dev-mcp buzz-mcp-launch git-credential-nostr buzz; do
